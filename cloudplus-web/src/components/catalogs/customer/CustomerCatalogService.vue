<template>
  <div class="product-container">
      <div class="product-header">
        <table class="table is-fullwidth" @click="toggleProductItemsVisibility">
          <tr>
            ...
            <td class="product-active-packages-count">
              <i class="fa fa-list" />
              <span>{{activePackagesCount > 0 ? activePackagesCount : 'no'}} Activated Packages</span>
            </td>
            <td></td>
            <!-- <td class="product-lowest-price has-text-centered">
              <span v-if="!productAvailable">Inactive</span>
              <span v-else>Activated</span>
            </td>
            <td class="product-activated-date" v-if="productAvailable">
              {{formatDate(product.activatedDate)}}
            </td>
            <td class="product-details switch">
              <cloud-plus-switch
          ...
          <tbody>
            <tr v-for="(productItem, index) in product.productItems" v-bind:key="index" v-bind:style="{'border-left': '4px solid ' + (productItem.isAddon ? '#e7995f' : '#2d8ac2')}">
              <td class="product-item-name">
                ...
                <!-- <div class="product-description">{{productItem.description}}
                </div> -->
                <div class="product-description">{{productDescription(productItem)}}
                  <!-- <brand-color-primary class="link"><span @click="openModal(productItem)">View more </span></brand-color-primary> -->
                </div>
              </td>
              ...
              <td>
                ${{productItem.cost | formatPrice}} per URL
              </td>
              <td class="product-details switch">
                <cloud-plus-switch
                  @input="toggleAvailability"
      ...
      <create-cscp-url-modal @provisioningStarted="setProvisioningStatus(statusInProgress)" v-if="showProductActivationModal" :showModal="showProductActivationModal" @closeModal="closeProductActivationModal"></create-cscp-url-modal>
      <!-- <confirmation-modal
...
<script>
import { provisioningStatus, productIdConstants } from '@/assets/constants/commonConstants';
...
import CreateCscpUrlModal from '@/components/services/support/CreateCscpUrlModal';
...
export default {
  ...
  data() {
    return {
      ...
      productLoaded: false,
      /* TODO change productAvailable to object to check for various product
       and change functionality of toggle button correspondingly */
  ...
  methods: {
    ...
    getProductAvailability() {
      provisioningService.getCscpUrlAvailability(this.companyId)
        .then(response => {
          const cscpUrlProvisioningStatus = response.data.result;
          this.productAvailable =
          cscpUrlProvisioningStatus !== this.provisioningStatus.notProvisioned;
          this.productLoaded = true;
          if (cscpUrlProvisioningStatus === 1) {
            this.activePackagesCount += 1;
          }
        });
    },
...
td.product-active-packages-count {
  width: 16%;
}
