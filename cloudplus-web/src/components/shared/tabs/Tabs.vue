<template>
  <div :class="{ 'vue-bulma-tabs': true, [`is-layout-${layout}`]: true }">
    <div  :class="{ tabs: true, [`is-${size}`]: size, [`is-${alignment}`]: alignment, [`is-${type}`]: type, 'is-fullwidth': isFullwidth }">
      ...
      <tab-list>
        <li class="tabs-list" v-for="(tab, index) in tabPanes"
          role="tab"
          ...
          <a class="tab-anchor is-unselectable">
...
export default {
  ...
  props: {
    ...
    // for tabs which need to disallow changing tabs when changes need to be saved
