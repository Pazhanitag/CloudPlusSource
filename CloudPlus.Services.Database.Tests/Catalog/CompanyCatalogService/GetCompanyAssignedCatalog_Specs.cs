...
namespace CloudPlus.Services.Database.Tests.Catalog.CompanyCatalogService
{
    ...
    public class GetCompanyAssignedCatalog_Specs
    {
        // System Under Test
        ...
        {
            Func<Task> act = async () => { await _sut.GetCompanyAssignedCatalog(20); };
        ...
        {
            Func<Task> act = async () => { await _sut.GetCompanyAssignedCatalog(4); };
            act.Should().Throw<NoAssignedCatalogException>();
        }

        [Test]
        public async Task When_Company_Has_Assigned_Catalog_Should_Return_Assigned_Catalog()
        {
            var catalog = await _sut.GetCompanyAssignedCatalog(3);
            catalog.Name.Should().Be("Master's Assignable Catalog");
        }
    }
}
